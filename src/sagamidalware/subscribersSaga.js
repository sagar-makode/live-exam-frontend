...
function* getsubscribers(action) {
  try {
    ...
    const response = yield call(axios.get, `https://live-exam-backend.onrender.com/subcount/${testId}/${studentId}`);
...
function* subscribeToTeacherSaga(action) {
  try {
    ...
    yield call(axios.post, "https://live-exam-backend.onrender.com/subscribeToTeacher", action.payload, {
      headers: {
...
function* unsubscribeToTeacherSaga(action) {
  try {
    const token = sessionStorage.getItem('token');
      ...
      yield call(axios.post, "https://live-exam-backend.onrender.com/unsubscribetoTeacher", action.payload, {
        headers: {
          Authorization: `Bearer${token}`
        }
      });
...
function* fetchStudentSubcriptionsSaga() {
  try {
     ...
     const response = yield call(axios.get, "https://live-exam-backend.onrender.com/studentsubcriptions", {
       headers: {
         Authorization: `Bearer${token}`
       }
     });
...
function* fetchAllCreaterSaga() {
  try {
     ...
     // Make a request to fetch user data from the backend with token in headers
     const response = yield call(axios.get, "https://live-exam-backend.onrender.com/allcreater", {
      headers: {
        Authorization: `Bearer${token}`
      }
    });
