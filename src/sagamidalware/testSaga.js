...
function* fetchTeacherCreatedTestInBinSaga() {
  try {
     ...
     const token = sessionStorage.getItem('token');
     // Make a request to fetch user data from the backend with token in headers
     const response = yield call(axios.get, "https://live-exam-backend.onrender.com/trashdata", {
       headers: {
         Authorization: `Bearer${token}`
       }
     });
...
function* restoreTestDataSaga(action){
  try {
    const response = yield call(axios.put, "https://live-exam-backend.onrender.com/undodeletedtests", action.payload); 
